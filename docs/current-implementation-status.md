...
Review screen layout: responsive master-detail. While browsing, the invoice queue is a full-width table (invoice number, amount, status pill, created date, plus a row-count chip in the header); opening an invoice collapses it to a compact identifier+status rail beside the detail pane on `xl` screens, and below `xl` the detail pane replaces the list entirely (drill-in) with the "Review Queue" breadcrumb navigating back — so the detail is always immediately visible rather than stacking below the fold. `DataTable` supports an opt-in `fitContainer` mode for narrow master rails (sizes to the container instead of forcing a horizontal scrollbar). Inside the detail pane, the Validation/Files/Line-items/Similar-invoices blocks lay out with container-driven `auto-fit` grid columns (two columns only when the pane itself is wide enough, independent of viewport breakpoints), and list rows wrap long unbroken tokens (mono rule codes, ids) inside their borders instead of overflowing.
